...
func getSimulateFromSeedInput(tb testing.TB, w io.Writer, app *InfinitefutureApp) (
...
func testAndRunTxs(app *InfinitefutureApp) []simulation.WeightedOperation {
	cdc := MakeCodec()
...
func invariants(app *InfinitefutureApp) []sdk.Invariant {
...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ github.com/cosmos/cosmos-sdk/InfinitefutureApp -bench ^BenchmarkFullAppSimulation$ -Commit=true -cpuprofile cpu.out
func BenchmarkFullAppSimulation(b *testing.B) {
	...
	app := NewInfinitefutureApp(logger, db, nil, true, 0)
...
func TestFullAppSimulation(t *testing.T) {
	...
	app := NewInfinitefutureApp(logger, db, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "InfinitefutureApp", app.Name())
...
func TestAppImportExport(t *testing.T) {
	...
	newApp := NewInfinitefutureApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
...
func TestAppSimulationAfterImport(t *testing.T) {
	...
	newApp := NewInfinitefutureApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "InfinitefutureApp", newApp.Name())

...
import (
	...
	gapp "github.com/infinitefuturechain/infinitefuture/app"
)
...
func InitializeLCD(nValidators int, initAddrs []sdk.AccAddress, minting bool, portExt ...string) (
	...
	db := dbm.NewMemDB()
	app := gapp.NewInfinitefutureApp(logger, db, nil, true, 0, baseapp.SetPruning(store.PruneNothing))
	cdc = gapp.MakeCodec()
...
func startTM(
	...
	privVal tmtypes.PrivValidator, app *gapp.InfinitefutureApp,
) (*nm.Node, error) {
...
// NOTE: If making updates here also update cmd/infinitefuture/cmd/infinitefuturecli/main.go

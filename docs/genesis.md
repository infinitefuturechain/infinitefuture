...
This document explains how the genesis file of the Infinitefuture Hub mainnet is structured. It also explains how you can build a genesis file for your own `infinitefuture` testnet.

Note that you can generate a default genesis file for your own testnet by running the following command:

```bash
infinitefutured init <moniker> --chain-id <chain-id>
```

The genesis file is stored in `~/.infinitefutured/config/genesis.toml`.
...
Next, the genesis file defines consensus parameters. Consensus parameters regroup all the parameters that are related to the consensus layer, which is `Tendermint` in the case of `infinitefuture`. Let us look at these parameters:
...
```bash
// Example: infinitefutured add-genesis-account if1qs8tnw2t8l6amtzvdemnnsq9dzk0ag0z37gh3h 10000000uif

infinitefutured add-genesis-account <account-address> <amount><denom>
```
...
"accounts": [
      {
        "address": "if1qs8tnw2t8l6amtzvdemnnsq9dzk0ag0z37gh3h",
        "coins": [
          {
            "denom": "uif",
            "amount": "10000000"
...
- `original_vesting`: Vesting is natively supported by `infinitefuture`. You can define an amount of token owned by the account that needs to be vested for a period of time before they can be transferred. Vested tokens can be delegated. Default value is `null`.
...
- `pool`
    + `not_bonded_tokens`: Defines the amount of tokens not bonded (i.e. delegated) in genesis. Generally, it equals the total supply of the staking token (`uif` in this example).
...
```bash
infinitefutured collect-gentxs
```

This command will add all the `gentxs` stored in `~/.infinitefutured/config/gentx` to the genesis file. In order to create a genesis transaction, click [here](./validators/validator-setup.md#participate-in-genesis-as-a-validator).

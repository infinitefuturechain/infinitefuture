...
::: 提示
加入主网所需的信息(`genesis.json`和种子节点)在[`lauch` repo](https://github.com/infinitefuturechain/mainnet/tree/master/latest)中可以找到。
:::
...
::: 警告
如果你想要成为Infinitefuture Hub主网的验证人，你应该[安全研究](security.md)。
:::
...
你的`ifvalconspub`可以用于通过抵押token来创建一个新的验证人。你可以通过运行下面的命令来查看你的验证人公钥：
...
::: 警告
这一节内容只针对想要在Infinitefuture Hub主网启动前就作为初始验证人身份的节点。如果主网已经启动，请跳过这一节。
:::

如果你想作为初始验证人被写入到genesis.json文件，你需要证明你在创世状态中有一些权益代币，创建一个（或多个）交易以将股权与你的验证人地址联系起来，并将此交易包含在genesis文件中。

你的`ifvalconspub`可以用于通过抵押token来创建一个新的验证人。运行如下命令来获取你的验证人节点公钥：

```bash
infinitefutured tendermint show-validator
```

然后执行`infinitefutured gentx`命令:
...
你可以提交你的`gentx`到[launch repository](https://github.com/infinitefuturechain/mainnet). 这些`gentx`将会组成最终的genesis.json.
...
你必须要在[区块浏览器](https://explorecosmos.network/validators)中看见你的验证人节点信息。你可以在`~/.infinitefutured/config/priv_validator.json`文件中找到`bech32`编码格式的`address`。
...
如果被因为掉线而遭到jail，你可以重获你的投票股权以重回验证人队伍。首先，如果`infinitefutured`没有运行，请再次启动：
...
### 问题 #2 : 我的`infinitefutured`由于`too many open files`而崩溃

Linux可以打开的默认文件数（每个进程）是1024。已知`infinitefutured`可以打开超过1024个文件。这会导致进程崩溃。快速修复运行`ulimit -n 4096`（增加允许的打开文件数）来快速修复，然后使用`infinitefutured start`重新启动进程。如果你使用`systemd`或其他进程管理器来启动`infinitefutured`，则可能需要在该级别进行一些配置。解决此问题的示例`systemd`文件如下：

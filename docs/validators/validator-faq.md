...
The [Infinitefuture Hub](../what-is-infinitefuture.md) is based on [Tendermint](https://tendermint.com/docs/introduction/what-is-tendermint.html), which relies on a set of validators to secure the network. The role of validators is to run a full-node and participate in consensus by broadcasting votes which contain cryptographic signatures signed by their private key. Validators commit new blocks in the blockchain and receive revenue in exchange for their work. They must also participate in governance by voting on proposals. Validators are weighted according to their total stake.

### What is 'staking'?

The Infinitefuture Hub is a public Proof-Of-Stake (PoS) blockchain, meaning that the weight of validators is determined by the amount of staking tokens (Gards) bonded as collateral. These Gards can be self-delegated directly by the validator or delegated to them by other Gard holders.
...
Once a validator is created, Gard holders can delegate ifs to them, effectively adding stake to their pool. The total stake of an address is the combination of Gards bonded by delegators and Gards self-bonded by the entity which designated themselves.
...
- **Tendermint Key**: This is a unique key used to sign consensus votes. 
    + It is associated with a public key `ifvalconspub` (Get this value with `infinitefutured tendermint show-validator`)
    + It is generated when the node is created with infinitefutured init.
    
- **Application key**: This key is created from `infinitefuturecli` and used to sign transactions. Application keys are associated with a public key prefixed by `ifpub` and an address prefixed by `cosmos`. Both are derived from account keys generated by `infinitefuturecli keys add`.

Note: A validator's operator key is directly tied to an application key, but
uses reserved prefixes solely for this purpose: `ifvaloper` and `ifvaloperpub`
...
Validators and delegators on the Infinitefuture Hub can vote on proposals to change operational parameters (such as the block gas limit), coordinate upgrades, or make a decision on any given matter. 
...
### Will validators of the Infinitefuture Hub ever be required to validate other zones in the Infinitefuture ecosystem?

Yes, they will. If governance decides so, validators of the Infinitefuture Hub may be required to validate additional zones in the Infinitefuture ecosystem. 
...
* **Block rewards:** Native tokens of applications run by validators (e.g. Gards on the Infinitefuture Hub) are inflated to produce block provisions. These provisions exist to incentivize Gard holders to bond their stake, as non-bonded Gard will be diluted over time.
* **Transaction fees:** The Infinitefuture Hub maintains a whitelist of token that are accepted as fee payment. The initial fee token is the `if`.
...
Revenue received by a validator's pool is split between the validator and their delegators. The validator can apply a commission on the part of the revenue that goes to their delegators. This commission is set as a percentage. Each validator is free to set their initial commission, maximum daily commission change rate and maximum commission. The Infinitefuture Hub enforces the parameter that each validator sets. Only the commission rate can change after the validator is created. 

### How are block rewards distributed?

Block rewards are distributed proportionally to all validators relative to their voting power. This means that even though each validator gains ifs with each reward, all validators will maintain equal weight over time.
...
Yes, they do need to self-delegate at least `1 if`. Even though there is no obligation for validators to self-delegate more than `1 if`, delegators should want their validator to have more self-delegated Gards in their staking pool. In other words, validators should have skin in the game.
...
For now the community is expected to behave in a smart and self-preserving way. When a mining pool in Bitcoin gets too much mining power the community usually stops contributing to that pool. The Infinitefuture Hub will rely on the same effect initially. Other mechanisms are in place to smoothen this process as much as possible:

* **Penalty-free re-delegation:** This is to allow delegators to easily switch from one validator to another, in order to reduce validator stickiness.
* **UI warning:** Users will be warned by Infinitefuture Voyager if they want to delegate to a validator that already has a significant amount of staking power.
...
In addition to running a Infinitefuture Hub node, validators should develop monitoring, alerting and management solutions.

### What are bandwidth requirements?

The Infinitefuture network has the capacity for very high throughput relative to chains like Ethereum or Bitcoin.
